...
class MBConvBlock(layers.Layer):
    ...
    def __init__(self, ksize, input_filters, output_filters, expand_ratio=1, stride=1, name=None):
        ...
        k = int(self.kernel_size)  # 将浮点数转换为整数
        s = int(self._stride)  # 将浮点数转换为整数

        self._depthwise_conv = layers.Conv2D(filters=oup,  kernel_size=3, strides=s, padding='same',
                                             use_bias=False)
...
if __name__ == '__main__':
    input = tf.random.normal((25,54,1,32))
    mbconv = MBConvBlock(ksize=3, input_filters=32, output_filters=32)
    out = mbconv(input)
    print(out.shape)
