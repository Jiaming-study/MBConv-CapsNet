...
def sparsemax(logits, axis):
    ...
    d = tf.shape(logits)[1]
    # 将 logits 排序
    logits_sorted = tf.sort(logits, axis=axis, direction='DESCENDING')
    logits_cumsum = tf.cumsum(logits_sorted, axis=axis)
    # 计算 rho
    indices = tf.cast(tf.range(1, d + 1), logits.dtype)  # 显式指定dtype为logits的数据类型
    indices_expanded = tf.expand_dims(indices, axis=-1)  # 扩展维度以匹配logits_sorted的维度
    rho = tf.cast(tf.argmax(tf.cast(indices_expanded * logits_sorted > logits_cumsum - 1, tf.int32), axis=axis) + 1, logits.dtype)
    # 计算 tau
    tau = (tf.reduce_sum(logits_sorted, axis=axis) - 1) / rho
    # 计算 sparsemax
    sparsemax = tf.maximum(logits - tau[:, tf.newaxis], 0)
    return sparsemax
def routing(u_hat_vecs, beta_a, iterations, output_capsule_num, i_activations):
    ...
    for i in range(iterations):
        if False:
            ...
            # leaky_routing = sparsemax(leaky_logits, dim=1)
            c = tf.split(leaky_routing, [1, output_capsule_num], axis=1)[1]
        else:
            # c = sparsemax(b, axis=1)
            c = softmax(b, 1)
        if i_activations is not None:
