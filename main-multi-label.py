...
from net import baseline_model_kimcnn, baseline_model_cnn,clstm_model, capsule_model_A, capsule_model_B, capsule_model_at, \
    capsule_model_atB, capsule_model_conv, capsule_model_convB, capsule_model_ALL, capsule_model_ALLB, capsule_model_at_sq, \
    capsule_model_at_sqB
...
parser.add_argument('--dataset', type=str, default='reuters_multi',
                    help='Options: reuters_multi,reuters_single')

parser.add_argument('--loss_type', type=str, default='margin_loss',
                    help='margin_loss, spread_loss, cross_entropy')

parser.add_argument('--model_type', type=str, default='capsule-ALL',
                    help='capsule-A,capsule-at,capsule-conv,KIMCNN')
...
# label = ['-1','world','sprorts','business','sci/tech']
label = map(str,label)
args.max_sent = 140
threshold = 0.5

X = tf.compat.v1.placeholder(tf.int32, [args.batch_size, args.max_sent], name="input_x")
y = tf.compat.v1.placeholder(tf.int64, [args.batch_size, args.num_classes], name="input_y")
...
if args.embedding_type == 'rand':#随机嵌入
...
if args.embedding_type == 'static':#静态嵌入
    W1 = tf.Variable(w2v, trainable = False)
    X_embedding = tf.nn.embedding_lookup(W1, X)
    X_embedding = X_embedding[...,tf.newaxis]
if args.embedding_type == 'nonstatic':#非静态嵌入
    W1 = tf.Variable(w2v, trainable = True)
    X_embedding = tf.nn.embedding_lookup(W1, X)
    X_embedding = X_embedding[...,tf.newaxis]
if args.embedding_type == 'multi-channel':#多通道嵌入
    W1 = tf.Variable(w2v, trainable = True)
...
tf.compat.v1.logging.info("input dimension:{}".format(X_embedding.get_shape()))
#---
...
if args.model_type == 'capsule-atB':
    poses, activations = capsule_model_atB(X_embedding, args.num_classes)
if args.model_type == 'capsule-conv':
    poses, activations = capsule_model_conv(X_embedding, args.num_classes)
if args.model_type == 'capsule-convB':
    poses, activations = capsule_model_convB(X_embedding, args.num_classes)
if args.model_type == 'capsule-ALL':
    poses, activations = capsule_model_ALL(X_embedding, args.num_classes)
if args.model_type == 'capsule-ALLB':
    poses, activations = capsule_model_ALLB(X_embedding, args.num_classes)
if args.model_type == 'capsule-at-sq':
    poses, activations = capsule_model_at_sq(X_embedding, args.num_classes,X_adj)
if args.model_type == 'capsule-at-sqB':
    poses, activations = capsule_model_at_sqB(X_embedding, args.num_classes,X_adj)
...
if args.model_type == 'CLSTM':
    poses, activations = clstm_model(X_embedding, args.max_sent, args.num_classes)
if args.loss_type == 'spread_loss':#传播损失
    loss = spread_loss(y, activations, margin)
if args.loss_type == 'margin_loss':#分类损失
    loss = margin_loss(y, activations)
if args.loss_type == 'cross_entropy':#交叉熵损失
...
for epoch in range(args.num_epochs):
    ...
    ER = exact_match_ratio(y_list, preds_probs)
    acc = accuracy_score(y_list, preds_probs)
    print ('\racc: %.3f' % acc,'\rER: %.3f' % ER, 'Precision: %.3f' % precision, 'Recall: %.3f' % recall, 'F1: %.3f' % F1)
